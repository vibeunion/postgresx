...
```bash
npx -y @goalforge/cli@latest init --goal "<design goal>" --config "/Volumes/Data/workspace/pgredis/.agents/goal-forge/goal-forge.config.json" --out "/Volumes/Data/workspace/pgredis/.agents/goal-forge/runs/<run-id>"
```

Run a deterministic local round:

```bash
npx -y @goalforge/cli@latest run "/Volumes/Data/workspace/pgredis/.agents/goal-forge/runs/<run-id>" --rounds 1 --adapter local
npx -y @goalforge/cli@latest validate "/Volumes/Data/workspace/pgredis/.agents/goal-forge/runs/<run-id>" --strict
```

Run repository-aware verification through the Codex adapter:

```bash
npx -y @goalforge/cli@latest run "/Volumes/Data/workspace/pgredis/.agents/goal-forge/runs/<run-id>" --rounds 1 --adapter codex --repo "/Volumes/Data/workspace/pgredis" --model gpt-5.3-codex
```
